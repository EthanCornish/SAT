...
def ListCardsCreate(setFile):
    ...
    # Getting the setName from the first line of the file
    # Name is not used anymore however the file format remains the same
    ...
    # Creates the list by going through each line in the file and add int each line in as a sublist
    listCards = []
    for line in file:
        line = line.strip('\n')
        line = line.split(',')
